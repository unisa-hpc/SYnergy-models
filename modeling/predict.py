# This is Python script to predict frequency of cloverleaf application by using the selected ML algorithm
...
training_file_normalized_loc = f"{script_dir}/../training-dataset/data/"  # frequency is not normalized
...
if __name__ == '__main__':
    ...
    training_kernel_name = list(map(lambda x: os.path.basename(x), os.listdir(test_file_normalized_loc)))
    test_kernel_name = list(map(lambda x: os.path.basename(x), os.listdir(training_file_normalized_loc)))
    ...
    for kernel_i in range(0, len(test_kernel_name)):
        ...
        if not df_test_data.empty:
            df_list_cloverleaf.append(df_test_data.loc[:, 'kernel_name':'flt_add'])

    df_cloverleaf = pd.concat(df_list_cloverleaf)
    kernel_num = len(df_cloverleaf)   # the total number of kernels

    df_cloverleaf_features = df_cloverleaf.loc[:, 'mem_gl':'flt_add']
    ...
    Time_clk_list = func_ML_Predict_CLK(x_model_train_feature, df_train_obj_time, Lasso(alpha=0.005),
                                        df_cloverleaf_features, df_gpu_clk, kernel_num)
    ...
    ES_clk_list, Loc_Freq_Min_Energy, Loc_Freq_Default = \
        func_ES_clk_prediction(x_model_train_feature, df_train_obj_energy,
                               ...
                               df_cloverleaf_features, df_gpu_clk, kernel_num, Energy_saving_value)  # objective
    ...
    # ES_clk_df.to_csv('output_cloverleaf_frequency_prediction_ES.csv')
    ...
    PL_clk_list = func_PL_clk_prediction(x_model_train_feature, df_train_obj_time, Lasso(alpha=1.0),
                                         df_cloverleaf_features, df_gpu_clk, kernel_num, Energy_saving_value,
    ...
    # PL_clk_df.to_csv('output_cloverleaf_frequency_prediction_PL.csv')
    # ===========================================================================================================
    # generate output file, .csv
    df_output = df_cloverleaf[['kernel_name']]
    ...
    ES_clk_df = ES_clk_df.reindex(index=df_output.index.copy())
    PL_clk_df = PL_clk_df.reindex(index=df_output.index.copy())
